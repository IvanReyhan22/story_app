...
class StoryRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            val responseData = apiService.getAllStories(0, page, state.config.pageSize).listStory
            ...
            database.withTransaction {
                ...
                database.storyDao().insertStory(responseData.filterNotNull().map { story ->
                    val (photoUrl, createdAt, name, description, lon, id, lat) = story
                    StoriesEntity(id!!, photoUrl, createdAt, name, description, lon, lat)
                })
            }
    ...
    fun invalidate(){
        database.storyDao().getAllStory().invalidate()
    }
